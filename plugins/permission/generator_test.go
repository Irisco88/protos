	...
	f.Var().Id("UserServicePermissions").Op("=").Op("&").Id("ServicePermission").Values(jen.Dict{
		...
		jen.Id("MethodStreams"): jen.Map(jen.String()).Op("*").Id("MethodStreamInfo").Values(jen.Dict{
			jen.Lit("/user.v1.UserService/signup"): jen.Op("&").Id("MethodStreamInfo").Values(jen.Dict{
				...
				jen.Id("Roles"): jen.Index().Qual("github.com/openfms/protos/gen/common/v1", "UserRole").Values(
					jen.Qual("github.com/openfms/protos/gen/common/v1", "UserRole_USER_ROLE_NORMAL"),
					jen.Qual("github.com/openfms/protos/gen/common/v1", "UserRole_USER_ROLE_READER"),
				),
			}),
		}),
	})
